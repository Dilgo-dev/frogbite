...
pub(super) fn draw_ws_replay_popup(frame: &mut Frame, app: &App) {
    let area = frame.area();
    let popup_w = area.width.saturating_sub(10).min(80);
    let popup_h: u16 = 7;
    let x = (area.width.saturating_sub(popup_w)) / 2;
    ...
    let mut lines = vec![
        Line::from(Span::styled(
            "  path to session JSON (replays sent messages)",
            Style::default().fg(muted()),
        )),
        Line::from(vec![
            Span::raw("  "),
            Span::styled(
                format!("{}\u{2588}", app.ws.replay_buffer),
                Style::default().fg(fg()),
            ),
        ]),
        Line::default(),
    ];
    if let Some(err) = &app.ws.replay_error {
        lines.push(Line::from(Span::styled(
            format!("  ! {err}"),
            Style::default().fg(red()),
        )));
